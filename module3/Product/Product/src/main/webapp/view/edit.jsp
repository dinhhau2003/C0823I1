...
<div class="container">
    <form action="/product?action=update" method="post" class="w-50 ms-5">
        ...
        <div class="form-floating mb-3">
            <input type="hidden" id="id" name="id" class="form-control" placeholder="Nhập Id" value="${product.getId()}"
                   readonly>
        ...
        <div class="mb-3">
            ...
            <select name="manufactor" id="manufactor" value="">
                <c:forEach var="manufactor" items="${manufactors}">
                    <option value="${manufactor.getManufactor_id()}">${manufactor.getName_manufactor()}</option>
                </c:forEach>
                -- Chọn Nhà sản xuất --
            </select>
        </div>
        <button class="btn btn-success" type="submit">Cập nhật</button>
        <a type="button" class="btn btn-primary" href="/product?action=list">Quay lại</a>    </form>
</div>
<script>
